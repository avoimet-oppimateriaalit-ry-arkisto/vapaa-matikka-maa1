...
Sievennä
	\begin{alakohdat}
	\alakohta{Eräs bakteerikanta kasvaa päivässä $75$ prosentilla. Montako prosenttia bakteerikannan alkuarvo $1$ g on bakteerikannan määrästä kolmen päivän kuluttua?}
	\alakohta{Mursupuvun hinnasta puolet tulee valmistuskuluista. Valmistuskuluista $75 \%$ on materiaalikuluja. Montako prosenttia materaalikulujen pitää laskea, jotta mursupuvun hinta tippuu $10$ \%?}
	\end{alakohdat}
	\begin{vastaus}
	 \begin{alakohdat}
	  \alakohta{$536 \%$}
...
%
	\begin{tehtava}
Määritellään funktio $f$ lausekkeella \[f(x)=2x^2-3x-2\] ja funktio $h$ lausekkeella \[h(x)=5x^{3}-12x^{-2}-2\].
	\begin{alakohdat}
	  \alakohta{Laske $h(2)$ ja $f(7)$}
	  \alakohta{Tutki kuvaajasta, mitkä ovat funktion $f(x)=2x^2-3x-2$ nollakohdat. Millä muuttujan arvoilla 
		    $f$ saa arvon $3$? Entä mikä on funktion $f$ arvo, kun $x=1$?}
	  \alakohta{Selvitä vielä $f(0)$ ja $h(0)$.}
	\end{alakohdat}
	\begin{vastaus}
	 \begin{alakohdat}
	  \alakohta{$h(2)=35$ ja $f(7) = 75$.}
	  \alakohta{Nollakohdat ovat $x = -\frac{1}{2}$ ja $x = 2$, $f$ saa arvon $3$ kun $x=-1$ tai $x=2\,\frac{1}{2}$, $f(1) = -3$.}
	  \alakohta{$f(0)=-2$ ja $h(0)$ ei ole määritelty.}
	 \end{alakohdat}
	\end{vastaus}
	\end{tehtava}
	\begin{center}
		\begin{kuvaajapohja}{0.75}{-3}{5}{-4}{4}
			\kuvaaja{2*x**2-3*x-2}{$f(x)=2x^2-3x-2$}{black}
		\end{kuvaajapohja}
	\end{center}
%muokkasin vähän jaakko viertiö 22.3.2014
%
	\begin{tehtava}
...
Ratkaise
	...
	\end{tehtava}
	\begin{tehtava}
Muuta kohdissa a) ja b) desimaaliluvuksi, ja kohdissa c) ja d) murtopotensseiksi sieventäen.
	\begin{alakohdat}
	\alakohta{$\frac{3}{10}$}
	\alakohta{$\frac{3}{16}$}

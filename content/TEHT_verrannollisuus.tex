...
\begin{tehtava}
    ...
    \alakohdat{
        ...
        § neliön, jonka pinta-ala on 5, sivujen pituudet
...
\alakohdat{
§ Kultakimpaleen arvo $x$ on suoraan verrannollinen sen massaan $m$, eli mitä painavampi kimpale, sitä arvokkaampi.
...
§ Planeettojen toisiinsa aiheuttama vetovoima $F$ on suoraan verrannollinen planeettojen massojen summaan $m_1 +m_2$ ja kääntäen verrannollinen niiden välisen etäisyyden $r$ neliöön.
}
\begin{vastaus}
\alakohdat{
§ $ \frac{x}{m}=c$ tai $ x=cm$. 
§ $ xt=c $ tai $ x=\frac{c}{t}$.
§ $ \frac{Fr^2}{m_1+m_2}=c$ tai $ F=c\frac{m_1 +m_2}{r²} $
}
\end{vastaus}
\end{tehtava}
...
§ Eivät kumpaakaan, sillä niiden tulo eikä osamäärä ei ole vakio. 
§ Kääntäen verrannollisia
§ Kääntäen ja suoraan verrannollisia, sillä niiden tulo ja osamäärä ovat vakioita.
}
\end{vastaus}
\end{tehtava}
...
\begin{tehtava}
	...
	§ Samalla aikavälillä puun kuutiohinta (e/m$^3$) laski $35 \%$. Kuinka paljon puun arvo muuttui (prosentteina)?
	}
	\begin{vastaus}
	\alakohdat{
	§ Kasvoi $45\,\%$.
...
\begin{tehtava}
	\alakohdat{
		§ Pullareseptin (http://kotiliesi.fi/ruoka/reseptit/pullataikina) mukaan taikinaan
		tarvitaan $5$\,dl maitoa, $2$\,dl sokeria ja $1,5$\,l vehnäjauhoja. Tällöin
		saadaan $40$ pikkupullaa.
		§ Minerva haluaa leipoa $16$\,pullaa. Kuinka paljon hän tarvitsee maitoa, sokeria ja vehnäjauhoja?
		§ Jos Minervalla on käytössä $3$\,dl maitoa, kuinka monta pikkupullaa hän voi enintään tehdä?
	} 
	\begin{vastaus}
		\alakohdat{
...
§ Uudet työntekijät suostuvat tekemään 15 tunnin työpäivää. Kuinka monta paria kenkiä yhden työpäivän aikana valmistuu?
} 
\begin{vastaus}
\alakohdat{
§ Kolme tuntia

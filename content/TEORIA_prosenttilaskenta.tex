...
\hline 
 & & \\
\vtop{\hbox{\strut ''Kuinka monta prosenttia}\hbox{\strut $a$ on $b$:stä?''}}  & $\rightleftarrows$ & \vtop{\hbox{\strut ''Kuinka paljon ($a$) on}\hbox{\strut $p$\,\% luvusta $b$?''}}\\ 
$\downarrow$ &  & $\downarrow$ \\ 
\vtop{\hbox{\strut ''Mitä ($b$) saadaan, kun $a$}\hbox{\strut kasvaa/vähenee $p$ prosenttia?''}} & $\rightleftarrows$ & \vtop{\hbox{\strut ''Kuinka monta prosenttia}\hbox{\strut $a$ on suurempi/pienempi kuin $b$?''}}\\ 
& & \\
\hline
 & & \\
\vtop{\hbox{\strut $\dfrac{a}{b}=p\,\%$}}  & $\rightleftarrows$ & \vtop{\hbox{\strut $p\,\%\cdot b=a $}}\\ 
$\downarrow$ &  & $\downarrow$ \\ 
\vtop{\hbox{\strut $\left(1\pm \dfrac{p}{100}\right)a=b$}} & $\rightleftarrows$ & \vtop{\hbox{\strut $\dfrac{|a-b|}{b}=p\,\%$}}\\ 
 & &\\
\hline 
\end{tabular}
}

Ongelmat, joiden välillä on nuoli molempiin suuntiin, voidaan esittää ja käsitellä molemmin tavoin -- tehtävänasettelu voidaan esittää toisenlaisena, vaikka ratkotaankin edelleen samaaongelmaa. Ylärivin kaksi tapausta ovat yksinkertaisimmat tehtävänannot, ja ne käsitellään ensin. Alarivin kysymyksenasettelun formalisointi (lausekkeiksi ja yhtälöksi kirjoittaminen) vaatii ylärivin kysymysten tekniikkaa. (Kaavion jäsentely tukee myös kuitenkin sitä, että ensin voitaisiin opetella vasemmalla puolella olevat ja sen jälkeen erillisesti oikealla puolella olevat -- tai toisinpäin.)

Tehtävän ratakisu saattaa vaatia edellä mainittujen tilanteiden yhdistelemistä ja tuntemattoman ratkaisemista yhtälöstä. Erilaisten suhteiden prosentuaalisille ilmaisuille on vakiintunut omat nimensä, jotka kannattaa opetella.

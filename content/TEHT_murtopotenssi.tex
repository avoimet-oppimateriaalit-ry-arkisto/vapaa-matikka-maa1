...
\begin{tehtava}
	...
	\begin{vastaus}
		\alakohdat{
			...
			§ Arvot vakiintuvat, kun $0,00659 \ldots \leq x \leq 1,444667\ldots$. Tämän voi esittää muodossa $\frac{1}{e^e} \leq x \leq \sqrt[e]{e}$, missä $e$ on irrationaalivakio nimeltään Neperin luku. Desimaalikehitelmänä $e = 2,718281828459\ldots$
		}
\end{vastaus}
\end{tehtava}
...
\begin{tehtava} %jonnekin yhtälölukuun
	$\star$ Potenssitornissa $x^{x^{x^{x^{\mathstrut^{.^{.^{.}}}}}}} $
		on äärettömän monta päällekkäistä eksponenttia. Yhtälöllä $ x^{x^{x^{x^{\mathstrut^{.^{.^{.}}}}}}} =2$ on yksi ratkaisu. Etsi se.
\begin{vastaus}
	$x = \sqrt{2}$
\end{vastaus}
\end{tehtava}
